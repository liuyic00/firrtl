...
class AssertionBasedSlice extends Transform with RegisteredTransform with DependencyAPIMigration {
  override def prerequisites = firrtl.stage.Forms.LowForm ++ Seq(Dependency[firrtl.transforms.DeadCodeElimination])
  ...
  def run(state: CircuitState, dontTouches: Seq[LogicNode], doTouchExtMods: Set[String]): CircuitState = {
    ...
    val dontTouchesSet = dontTouches.toSet
    ...
    val dontTouchesandDeps = dontTouchesSet.flatMap(depGraph.reachableFrom) ++ dontTouchesSet

    val liveNodes = assertAssumeVars.flatMap(depGraph.reachableFrom) ++
      assertAssumeVars ++
      dontTouchesandDeps ++
      Seq(circuitSink)
